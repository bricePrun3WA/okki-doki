...
function Login(props) {
    ...
    // Informations sur la session actuelle dans Redux
    ...
    // Hook pour la gestion de redirection
        ...
        fetch(reqLogin)
            ...
            .then(data => {
                ...
                if (data.hasError) {
                    ...
                    return;
                // Connexion ADMIN effectuée
                } else if (data?.isLogged) {
                    dispatch(connexion({isLogged: true, espace: "admin"}));
                }
            })
            // Redirection vers la page d'accueil lors de la validation de connexion
            .then(result => {
                if (reducerSession?.isLogged?.admin) {
                    navigate('/admin');
                }
            })
            // Erreur de conneixon
            ...
            <article className="column">
                ...
                <form method="post" className="white-box" onSubmit={(e) => { submitLogin(e, user) }}>
                    <div className="margin">
                        ...
                        <input name="email" type="email" className="inputUser" value={user.email} onChange={handleChange} />
                    </div>
                    <div className="margin">
                        <label htmlFor="pwd">Mot de passe</label>
                        <input name="pwd" type="password" className="inputUser" value={user.pwd} onChange={handleChange} />
                    </div>
